...
Class Flux2cat{
	...
	function createFluxCat($expname){
		...
		//get today's mjd
		$mjdnow = gregoriantojd(gmdate("m"),gmdate("d"),gmdate("Y")) - 2400000;
		...
		if (isset($expname)){
			...
			//new* select from 1/2 year records
			$difmjd = $mjdnow - 183;
			$dbq = $db->query("dynob",'SELECT * FROM Fluxmonx WHERE mjd > '.$difmjd.' ORDER BY mjd DESC');
			for ($u = 0; $u<count($dbq); $u++){
				$items0 = array_slice($dbq[$u],4); // (remove id, session, mjd from array)
				foreach($items0 as $source => $flux){
					//replace if empty
					if($flux !=="" || $flux !== NULL){
						if($dbq[0][$source] == NULL || $dbq[0][$source] == ""){
							$dbq[0][$source] = $flux;
						}
					}
				}
		...
		if (isset($expname)){
			...
			//new* select from 1/2 year records
			$difmjd = $mjdnow - 183;
			$dbq = $db->query("dynob",'SELECT * FROM Fluxmons WHERE mjd > '.$difmjd.' ORDER BY mjd DESC');
			for ($u = 0; $u<count($dbq); $u++){
				$items0 = array_slice($dbq[$u],4); // (remove id, session, mjd from array)
				foreach($items0 as $source => $flux){
					//replace if empty
		...
		$file = new SplFileObject($_SERVER['DOCUMENT_ROOT']."scheduling/dynflux/myfluxmodel.cat");
		$newfluxcat = new SplFileObject($_SERVER['DOCUMENT_ROOT']."scheduling/dynflux/dynflux.cat", "w");
		...
		while ( ! $file->eof()) {
			...
			if ($line[0]==="*" || $line[0]==" "){
				...
				if(array_key_exists($_source,$refluxmodel)){
					...
					elseif($_type == "B"){
						$refluxcatline = str_pad(trim($_source),9," ").str_pad(trim($_band),2," ").str_pad(trim($_type),2," ").str_pad("0.0",5," ").str_pad($refluxmodel[$_source],5," ")."13000.0";
